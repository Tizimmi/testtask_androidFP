	...
	public static class ReflectionTypeAnalyzer
	{
		...
		static List<ReflectionTypeInfo.InjectPropertyInfo> GetPropertyInfos(Type type)
		{
			return type.DeclaredInstanceProperties()
				...
				.Select(x => new ReflectionTypeInfo.InjectPropertyInfo(x, GetInjectableInfoForMember(type, x)))
				.ToList();
		}
		...
		static List<ReflectionTypeInfo.InjectFieldInfo> GetFieldInfos(Type type)
		{
			return type.DeclaredInstanceFields()
				.Where(x => _injectAttributeTypes.Any(a => x.HasAttribute(a)))
				.Select(x => new ReflectionTypeInfo.InjectFieldInfo(x, GetInjectableInfoForMember(type, x)))
				.ToList();
		}
		...
		static List<ReflectionTypeInfo.InjectMethodInfo> GetMethodInfos(Type type)
		{
			...
			for (var i = 0; i < methodInfos.Count; i++)
			{
				...
				var injectParamInfos = methodInfo.GetParameters().Select(x => CreateInjectableInfoForParam(type, x)).ToList();
		...
		static ReflectionTypeInfo.InjectParameterInfo CreateInjectableInfoForParam(Type parentType, ParameterInfo paramInfo)
		{
			...
			var isOptionalWithADefaultValue = (paramInfo.Attributes & ParameterAttributes.HasDefault) == ParameterAttributes.HasDefault;

			return new ReflectionTypeInfo.InjectParameterInfo(paramInfo,
				new InjectableInfo(isOptionalWithADefaultValue || isOptional,
					identifier,
					paramInfo.Name,
		...
		static InjectableInfo GetInjectableInfoForMember(Type parentType, MemberInfo memInfo)
		{
			...
			var memberType = memInfo is FieldInfo ? ((FieldInfo) memInfo).FieldType : ((PropertyInfo) memInfo).PropertyType;

			return new InjectableInfo(isOptional,
				identifier,
				memInfo.Name,
				memberType,
				null,
