...
namespace Zenject.Internal
{
	public static class ZenUtilInternal
	{
		...
		public static IEnumerable<SceneContext> GetAllSceneContexts()
		{
			...
			{
				var contexts = scene.GetRootGameObjects().SelectMany(root => root.GetComponentsInChildren<SceneContext>()).ToList();
		...
		static void GetInjectableMonoBehavioursUnderGameObjectInternal(GameObject gameObject, List<MonoBehaviour> injectableComponents)
		{
			if (gameObject == null)
				return;
		...
		public static IEnumerable<GameObject> GetRootGameObjects(Scene scene)
		{
			...
			{
				if (scene.isLoaded)
					return scene.GetRootGameObjects().Where(x => x.GetComponent<ProjectContext>() == null);
				...
				return Resources.FindObjectsOfTypeAll<GameObject>()
					.Where(x => x.transform.parent == null && x.GetComponent<ProjectContext>() == null && x.scene == scene);
			}
		}
