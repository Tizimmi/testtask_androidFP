...
namespace Zenject
{
	...
	public static class TypeAnalyzer
	{
		...
		static readonly Dictionary<Type, InjectTypeInfo> _typeInfo = new Dictionary<Type, InjectTypeInfo>();
		...
		static readonly Dictionary<Type, bool> _allowDuringValidation = new Dictionary<Type, bool>();
...
#endif
			{
				var getInfoMethod = type.GetMethod(ReflectionBakingGetInjectInfoMethodName,
		...
		{
			return type == null || type.IsEnum() || type.IsArray || type.IsInterface() || type.ContainsGenericParameters() || IsStaticType(type) ||
					type == typeof(object);
		}
		...
		static InjectTypeInfo CreateTypeInfoFromReflection(Type type)
		{
			...
			var injectConstructor = ReflectionInfoTypeInfoConverter.ConvertConstructor(reflectionInfo.InjectConstructor, type);

			var injectMethods = reflectionInfo.InjectMethods.Select(ReflectionInfoTypeInfoConverter.ConvertMethod).ToArray();

			var memberInfos = reflectionInfo.InjectFields.Select(x => ReflectionInfoTypeInfoConverter.ConvertField(type, x))
				.Concat(reflectionInfo.InjectProperties.Select(x => ReflectionInfoTypeInfoConverter.ConvertProperty(type, x)))
				.ToArray();
