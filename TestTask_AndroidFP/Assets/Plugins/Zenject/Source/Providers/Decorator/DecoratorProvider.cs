...
namespace Zenject.Internal
{
	...
	{
		readonly Dictionary<IProvider, List<object>> _cachedInstances = new Dictionary<IProvider, List<object>>();
		...
		object DecorateInstance(object instance, InjectContext context)
		{
			for (var i = 0; i < _decoratorFactories.Count; i++)
				instance = _decoratorFactories[i].Create(context.Container.IsValidating ? default(TContract) : (TContract) instance);
