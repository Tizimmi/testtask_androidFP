...
namespace Zenject
{
	...
	public class MemoryPoolBase<TContract> : IValidatable, IMemoryPool, IDisposable
	{
		...
		public void Resize(int desiredPoolSize)
		{
			...
			if (_settings.ExpandMethod == PoolExpandMethods.Disabled)
				throw new PoolExceededFixedSizeException(
					"Pool factory '{0}' attempted resize but pool set to fixed size of '{1}'!".Fmt(GetType(), _inactiveItems.Count));
		...
		void IValidatable.Validate()
		{
			...
			{
				throw new ZenjectException("Validation for factory '{0}' failed".Fmt(GetType()), e);
			}
		}

		[Inject]
		void Construct(IFactory<TContract> factory, DiContainer container, [InjectOptional] MemoryPoolSettings settings)
		{
		...
		TContract AllocNew()
		{
			try
			{
				...
				if (!_container.IsValidating)
				{
					Assert.IsNotNull(item,
						"Factory '{0}' returned null value when creating via {1}!",
						_factory.GetType(),
						GetType());
			...
			{
				throw new ZenjectException("Error during construction of type '{0}' via {1}.Create method!".Fmt(typeof(TContract), GetType()), e);
			}
		}
