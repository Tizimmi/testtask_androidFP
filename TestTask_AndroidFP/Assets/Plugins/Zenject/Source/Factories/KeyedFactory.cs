...
namespace Zenject
{
	public abstract class KeyedFactoryBase<TBase, TKey> : IValidatable
	{
		...
		public void Initialize()
		{
			...
			if (!duplicates.IsEmpty())
				throw Assert.CreateException("Found duplicate values in KeyedFactory: {0}", duplicates.Select(x => x.ToString()).Join(", "));
#endif
	...
	public class KeyedFactory<TBase, TKey, TParam1> : KeyedFactoryBase<TBase, TKey>
	{
		...
		{
			return (TBase) Container.InstantiateExplicit(GetTypeForKey(key),
