...
namespace Zenject
{
	...
	public abstract class ProviderBindingFinalizer : IBindingFinalizer
	{
		...
		public void FinalizeBinding(DiContainer container)
		{
			...
			{
				throw Assert.CreateException(e,
		...
		protected void RegisterProvider(DiContainer container, Type contractType, IProvider provider)
		{
			...
			container.RegisterProvider(new BindingId(contractType, BindInfo.Identifier),
				BindInfo.Condition,
				provider,
				BindInfo.NonLazy);
			...
			if (contractType.IsValueType() && !(contractType.IsGenericType() && contractType.GetGenericTypeDefinition() == typeof(Nullable<>)))
			{
				...
				container.RegisterProvider(new BindingId(nullableType, BindInfo.Identifier),
					BindInfo.Condition,
					provider,
					BindInfo.NonLazy);
			}
		...
		{
			var isConcreteOpenGenericType = concreteType.IsOpenGenericType();
			var isContractOpenGenericType = contractType.IsOpenGenericType();
