...
namespace Zenject
{
	...
	public class FactoryFromBinder<TParam1, TContract> : FactoryFromBinderBase
	{
		...
		{
			ProviderFunc = (container) => new MethodProviderWithContainer<TParam1, TContract>(method);
		...
		{
			return new FactorySubContainerBinder<TParam1, TContract>(BindContainer,
				BindInfo,
				FactoryBindInfo,
				subIdentifier);
		}
	...
	public static class FactoryFromBinder1Extensions
	{
			...
			Action<ConcreteBinderGeneric<IFactory<TParam1, TContract>>> factoryBindGenerator)
		{
			...
			fromBinder.ProviderFunc = (container) => { return new IFactoryProvider<TParam1, TContract>(container, factoryId); };

			return new ArgConditionCopyNonLazyBinder(fromBinder.BindInfo);
		}

		public static ArgConditionCopyNonLazyBinder FromPoolableMemoryPool<TParam1, TContract>(this FactoryFromBinder<TParam1, TContract> fromBinder)
			...
			where TMemoryPool : MemoryPool<TParam1, IMemoryPool, TContract>
		{
			...
			fromBinder.ProviderFunc = (container) => { return new PoolableMemoryPoolProvider<TParam1, TContract, TMemoryPool>(container, poolId); };
