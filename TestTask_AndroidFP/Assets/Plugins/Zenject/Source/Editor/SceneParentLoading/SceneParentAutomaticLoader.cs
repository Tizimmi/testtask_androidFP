...
namespace Zenject.Internal
{
	...
	public static class SceneParentAutomaticLoader
	{
		...
		static void OnPlayModeStateChanged(PlayModeStateChange state)
		{
			if (state == PlayModeStateChange.ExitingEditMode)
			{
				...
				catch (Exception e)
				{
					...
					throw new ZenjectException("Failure occurred when attempting to load default scene parent contracts!", e);
				}
			}
		...
		static Dictionary<string, LoadedSceneInfo> GetCurrentSceneContractsMap(List<LoadedSceneInfo> sceneInfos)
		{
			...
			Dictionary<string, string> defaultContractsMap)
		{
			...
			decoratedSceneInfo = LoadDefaultSceneForContract(sceneInfo, decoratedContractName, defaultContractsMap);
			...
			Dictionary<string, string> defaultContractsMap)
		{
			...
			{
				throw new ZenjectException("Error while attempting to load contracts for scene '{0}'".Fmt(sceneInfo.Scene.name), e);
			}

			return CreateLoadedSceneInfo(scene);
		}

		static void ValidateDecoratedSceneMatch(LoadedSceneInfo decoratorInfo, LoadedSceneInfo decoratedInfo)
		{
			...
			Assert.That(decoratorIndex < decoratedIndex,
				...
				decoratorInfo.Scene.name,
				decoratedInfo.Scene.name);

			if (activeIndex > decoratorIndex)
				EditorSceneManager.SetActiveScene(decoratorInfo.Scene);
		}

		static void ValidateParentChildMatch(LoadedSceneInfo parentSceneInfo, LoadedSceneInfo sceneInfo)
		{
		...
		static LoadedSceneInfo TryCreateLoadedSceneInfo(Scene scene)
		{
			...
			{
				Assert.IsNull(decoratorContext, "Found both SceneContext and SceneDecoratorContext in scene '{0}'", scene.name);
		...
		static void AddToContractMap(Dictionary<string, LoadedSceneInfo> contractMap, LoadedSceneInfo info)
		{
			...
			foreach (var contractName in info.SceneContext.ContractNames)
			{
				...
				if (contractMap.TryGetValue(contractName, out currentInfo))
					throw Assert.CreateException("Found multiple scene contracts with name '{0}'. Scene '{1}' and scene '{2}'",
						contractName,
						currentInfo.Scene.name,
						info.Scene.name);
