...
namespace Zenject.Internal
{
	public static class ZenUnityEditorUtil
	{
		...
		{
			var activeScenePaths = EditorBuildSettings.scenes.Where(x => x.enabled).Select(x => x.path).ToList();
		...
		public static void RunCurrentSceneSetup()
		{
			...
			foreach (var sceneContext in GetAllSceneContexts())
				...
				catch (Exception e)
				{
					...
					throw new ZenjectException("Scene '{0}' Failed To Start!".Fmt(sceneContext.gameObject.scene.name), e);
				}
		}
		...
		public static SceneContext GetSceneContextForScene(Scene scene)
		{
			...
			Assert.IsNotNull(sceneContext, "Could not find scene context for scene '{0}'", scene.name);
		...
		public static SceneContext TryGetSceneContextForScene(Scene scene)
		{
			...
			var sceneContexts = scene.GetRootGameObjects().SelectMany(x => x.GetComponentsInChildren<SceneContext>()).ToList();

			if (sceneContexts.IsEmpty())
				return null;

			Assert.That(sceneContexts.Count == 1, "Found multiple SceneContexts in scene '{0}'.  Expected a maximum of one.", scene.name);
		...
		public static SceneDecoratorContext GetDecoratorContextForScene(Scene scene)
		{
			...
			Assert.IsNotNull(decoratorContext, "Could not find decorator context for scene '{0}'", scene.name);
		...
		public static SceneDecoratorContext TryGetDecoratorContextForScene(Scene scene)
		{
			...
			var decoratorContexts = scene.GetRootGameObjects().SelectMany(x => x.GetComponentsInChildren<SceneDecoratorContext>()).ToList();

			if (decoratorContexts.IsEmpty())
				return null;

			Assert.That(decoratorContexts.Count == 1, "Found multiple DecoratorContexts in scene '{0}'.  Expected a maximum of one.", scene.name);
			...
			for (var i = 0; i < EditorSceneManager.sceneCount; i++)
			{
				...
				else if (decoratorContext != null)
				{
					Assert.That(!string.IsNullOrEmpty(decoratorContext.DecoratedContractName),
						"Missing Decorated Contract Name on SceneDecoratorContext in scene '{0}'",
						scene.name);
		...
		{
			return Path.Combine(Path.Combine(Path.GetFullPath(Application.dataPath), ".."), assetPath);
		}
		...
		{
			return ConvertFullAbsolutePathToAssetPath(GetCurrentDirectoryAbsolutePathFromSelection());
		}
		...
		public static void SaveScriptableObjectAsset(string path, ScriptableObject asset)
		{
		...
		{
			return GetSelectedPathsInProjectsTab().Where(x => Directory.Exists(x)).ToList();
		}
