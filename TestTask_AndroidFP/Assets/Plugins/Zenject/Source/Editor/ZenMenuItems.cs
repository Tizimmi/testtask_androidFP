...
namespace Zenject.Internal
{
	public static class ZenMenuItems
	{
		...
		public static void CreateDefaultSceneContractConfig()
		{
			...
			ZenUnityEditorUtil.SaveScriptableObjectAsset(Path.Combine(folderPath, DefaultSceneContractConfig.ResourcePath + ".asset"), config);
		}

		[MenuItem("Assets/Create/Zenject/Scriptable Object Installer", false, 1)]
		public static void CreateScriptableObjectInstaller()
		{
			AddCSharpClassTemplate("Scriptable Object Installer",
				"UntitledInstaller",
				"using UnityEngine;" + "\nusing Zenject;" + "\n" +
		...
		{
			AddCSharpClassTemplate("Mono Installer",
				"UntitledInstaller",
		...
		{
			AddCSharpClassTemplate("Installer",
				"UntitledInstaller",
		...
		{
			AddCSharpClassTemplate("Editor Window",
				"UntitledEditorWindow",
			...
			if (absoluteDir == null)
			{
				EditorUtility.DisplayDialog("Error",
					"Could not find directory to place the '{0}.prefab' asset.  Please try again by right clicking in the desired folder within the projects pane."
						.Fmt(ProjectContext.ProjectContextResourcePath),
					"Ok");
		...
		{
			return AddCSharpClassTemplate(friendlyName,
				defaultFileName,
				templateStr,
		...
		public static string AddCSharpClassTemplate(
			...
			string folderPath)
		{
			var absolutePath = EditorUtility.SaveFilePanel("Choose name for " + friendlyName,
				folderPath,
				defaultFileName + ".cs",
				"cs");
		...
		{
			AddCSharpClassTemplate("Unit Test",
				"UntitledUnitTest",
		...
		{
			AddCSharpClassTemplate("Integration Test",
				"UntitledIntegrationTest",
		...
		{
			AddCSharpClassTemplate("Scene Test Fixture",
				"UntitledSceneTest",
				"using Zenject;" + "\nusing System.Collections;" + "\nusing UnityEngine;" + "\nusing UnityEngine.TestTools;" + "\n" +
