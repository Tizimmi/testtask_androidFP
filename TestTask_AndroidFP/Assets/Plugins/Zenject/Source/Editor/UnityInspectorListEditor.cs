...
namespace Zenject
{
	public abstract class UnityInspectorListEditor : Editor
	{
		...
		public virtual void OnEnable()
		{
			...
			var infos = Enumerable.Range(0, names.Length)
				.Select(i => new
				{
					Name = names[i],
			...
			foreach (var info in infos)
			{
				...
				var installersList = new ReorderableList(serializedObject,
					installersProperty,
					true,
					true,
					true,
				...
				installersList.drawHeaderCallback += rect => { GUI.Label(rect, new GUIContent(closedName, closedDesc)); };
				installersList.drawElementCallback += (
					rect,
					index,
					active,
					focused) =>
