...
namespace Zenject
{
	...
	public class GuiRenderableManager
	{
		class RenderableInfo
		{
			...
			public readonly IGuiRenderable Renderable;
		...
		readonly List<RenderableInfo> _renderables;

		public GuiRenderableManager(
			[Inject(Optional = true, Source = InjectSources.Local)] List<IGuiRenderable> renderables,
			[Inject(Optional = true, Source = InjectSources.Local)] List<ValuePair<Type, int>> priorities)
		{
			...
			foreach (var renderable in renderables)
			{
				...
				var matches = priorities.Where(x => renderable.GetType().DerivesFromOrEqual(x.First)).Select(x => x.Second).ToList();
		...
		public void OnGui()
		{
			foreach (var renderable in _renderables)
				...
				{
					throw Assert.CreateException(e, "Error occurred while calling {0}.GuiRender", renderable.Renderable.GetType());
				}
		}
	}
}