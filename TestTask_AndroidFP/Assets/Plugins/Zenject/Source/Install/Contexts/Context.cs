...
namespace Zenject
{
	public abstract class Context : MonoBehaviour
	{
		...
		readonly List<InstallerBase> _normalInstallers = new List<InstallerBase>();
		readonly List<Type> _normalInstallerTypes = new List<Type>();
		...
		{
			InstallInstallers(_normalInstallers,
			...
			List<MonoInstaller> installerPrefabs)
		{
			...
			var allInstallers = normalInstallers.Cast<IInstaller>()
				...
				.Concat(installers.Cast<IInstaller>())
				.ToList();
		...
		protected void InstallSceneBindings(List<MonoBehaviour> injectableMonoBehaviours)
		{
			...
			foreach (var binding in Resources.FindObjectsOfTypeAll<ZenjectBinding>())
			{
				...
				if (this is SceneContext)
					if (binding.Context == null && binding.UseSceneContext && binding.gameObject.scene == gameObject.scene)
						binding.Context = this;
