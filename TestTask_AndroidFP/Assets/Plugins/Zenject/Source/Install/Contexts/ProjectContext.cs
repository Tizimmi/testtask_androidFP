...
namespace Zenject
{
	public class ProjectContext : Context
	{
		...
		public static GameObject TryGetPrefab()
		{
			...
			if (prefabs.Length > 0)
			{
				Assert.That(prefabs.Length == 1, "Found multiple project context prefabs at resource path '{0}'", ProjectContextResourcePath);
			...
			if (prefabs.Length > 0)
			{
				Assert.That(prefabs.Length == 1, "Found multiple project context prefabs at resource path '{0}'", ProjectContextResourcePathOld);
				return (GameObject) prefabs[0];
			}
		...
		static void InstantiateAndInitialize()
		{
			...
			Assert.That(FindObjectsOfType<ProjectContext>().IsEmpty(), "Tried to create multiple instances of ProjectContext!");
		...
		void InstallBindings(List<MonoBehaviour> injectableMonoBehaviours)
		{
			...
			_container.Bind(typeof(ProjectKernel), typeof(MonoKernel)).To<ProjectKernel>().FromNewComponentOn(gameObject).AsSingle().NonLazy();
